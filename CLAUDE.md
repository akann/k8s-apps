...
### ArgoCD has no public DNS record — CI triggers refresh directly, not via webhook

`argocd.yanatech.co.uk` has **no public DNS record at all** (confirmed 2026-07-18: `dig @1.1.1.1 argocd.yanatech.co.uk` returns empty, vs. real answers for `yanatech.co.uk`/`akan.nkweini.org`) — unlike the public app subdomains, it only resolves internally (CoreDNS/split-horizon, to the same ingress LB IP `192.168.33.200`). This looks deliberate: a GitHub webhook aimed at it would silently 502 forever since GitHub can never resolve or reach it — don't add one. A GitHub Actions `gh api repos/*/hooks` POST was tried and reverted the same day after this was discovered.

Instead, **`k8s-apps`, `yana-stocks`, `shared-services`, and `ml`** (the 4 repos with a self-hosted ARC runner — `akan`/`dove-house-tt`/`yanatech` don't have one and are stuck on plain polling) have a CI step that runs **on the self-hosted runner** and patches the relevant Application's `argocd.argoproj.io/refresh` annotation **directly via `kube-apiserver`**, using the runner pod's own auto-mounted ServiceAccount token — no ArgoCD API token needed. RBAC is in `infrastructure/argocd/ci-refresh-rbac.yaml`: one `Role`+`RoleBinding` pair per repo, each granting only `runners-<repo>-gha-rs-no-permission` (the actual identity ARC's ephemeral job pods run as — not the `runners-<repo>-*-listener` SA) `get`/`patch` on exactly its own named Application (`bootstrap` / `yana-stocks` / `shared-services-deployment` / `ml-deployment`), nothing else. Network path already worked with zero new NetworkPolicy — `actions-runner`'s existing `allow-kube-apiserver-egress` (ports 443/6443, `netpol-apiserver-egress.yaml`) already covers it, since kube-apiserver is a Cilium reserved-identity destination (unlike a plain ClusterIP such as `argocd-server.argocd.svc.cluster.local`, which timed out in testing — that's the same cross-namespace-ClusterIP gotcha as the Grafana→Prometheus case above, and going through kube-apiserver sidesteps it entirely rather than fixing it).

Where the CI step lives depends on whether the repo's existing `gitops` job (the one that commits the image-tag bump) already runs on the self-hosted runner:
- `yana-stocks`'s `gitops` job already does — the refresh call is appended inline, right after `git push`.
- `ml`/`shared-services`' `gitops` jobs run on `ubuntu-latest` (git-only, no cluster network) — a separate `argocd-refresh` job was added instead, `needs: gitops`, `runs-on: runners-<repo>`.
- `k8s-apps` had no per-push workflow at all — a new minimal one (`.github/workflows/argocd-refresh.yml`) triggers on every push to `main`.

**Caveat confirmed 2026-07-18, not fully solved:** the refresh annotation reliably reaches the Application object (verified via `kubectl auth can-i` + a real in-cluster test pod impersonating each SA), but how fast the controller actually *acts* on it varies a lot — one test on `harbor-backup` resolved in ~15s, but a same-day test on `yana-stocks` took ~165s, close to the default ~2min reconcile-cache TTL seen in controller logs (`"comparison expired, requesting refresh... expiry: 2m0s"`). With 64 Applications on a single non-sharded controller, a busy queue (e.g. from unrelated churn elsewhere in the cluster) can apparently delay a "refresh now" request almost as long as just waiting for the next poll would have. This still strictly beats plain polling — it removes the up-to-~3min *git-detection* latency entirely — but don't expect a guaranteed sub-30s deploy-to-sync time.

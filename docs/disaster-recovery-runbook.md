...
4. **Break-glass copy of the `./venv` Note, outside Vaultwarden.** Vaultwarden's own data lives in `pg-main`, recoverable per Phase 4 — but that's circular: you need these secrets to get ESO/Infisical running before you can even reach the point of restoring `pg-main`. If Vaultwarden is genuinely gone (not just down — destroyed along with Ceph), there must be a copy of the `./venv` contents somewhere that doesn't depend on the on-prem cloud at all. **Current answer: the same Note is also kept in LastPass** (a separate account, independent of this infrastructure) — update it there every time `regenerate-venv.sh` produces a real change, not just once. If that copy doesn't exist or is stale, that's a gap — fix it before you need this runbook for real.
...
**Quoting matters — don't hand-type values with plain double quotes.** `./venv` is `source`d by `bootstrap.sh`, so any value containing a space, `$`, backtick, or quote will get silently word-split or expanded if it's not shell-escaped properly — no error, just a truncated or corrupted secret landing on the cluster. Confirmed this actually happens (2026-07-18, caught during a deliberate stress test). `regenerate-venv.sh` already handles this correctly (every value goes through `printf '%q'`), so if the Note was populated from that script's output, you're fine. If you ever hand-edit a value in the Note directly, generate its safe form first rather than typing `export VAR="raw value"` by hand:

```bash
printf 'export VAR=%q\n' 'the raw value, single-quoted here is safe'
```

and paste that line's output into the Note instead of the raw value.
...
- **`./venv`-sourced secret-creation block**: exercised for real (2026-07-18) against a scratch namespace with deliberately adversarial fake values (spaces, `$`, backticks, embedded single quotes) — every `kubectl create secret`/`docker-registry`/`patch` pattern in `bootstrap.sh` ran twice to confirm idempotency (the `--dry-run=client -o yaml | kubectl apply -f -` pattern correctly updates rather than erroring on a second run) and every decoded value matched exactly. This is what caught the `regenerate-venv.sh` bug below — the test itself worked as intended. Still not run as part of an actual full `bootstrap.sh` execution against a real cluster, only as isolated command patterns.
- **Bug found and fixed 2026-07-18: `regenerate-venv.sh` wrote unquoted values.** `export VAR=$(...)` with no quoting at all — any secret value containing a space, `$`, backtick, or quote would get silently word-split or expanded when `bootstrap.sh` later `source`d the file, truncating or corrupting the value with no error. Caught by the stress test above. Fixed by routing every value through `printf '%q'`, which round-trip-tested exactly (verified with the same adversarial values, both in isolation and end-to-end through a real `kubectl create secret`).

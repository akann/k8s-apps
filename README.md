...
- `NetworkPolicy` objects work for most traffic, but **two categories require `CiliumNetworkPolicy`**:
  - **Ceph OSD egress** (bare-metal IPs, ports 6802-6809): use `toCIDR: 192.168.22.0/24`
  - **Cross-namespace ClusterIP routing**: standard NetworkPolicy doesn't reliably handle this in native routing mode — use `toEndpoints` with pod labels. Confirmed affected: Grafana → Prometheus, Grafana → PostgreSQL (yana-stocks)
...
graph LR
    ...
    subgraph ESO["External Secrets Operator"]
        CSS["ClusterSecretStore: infisical<br/>(Infisical provider — main store)"]
        CSS2["ClusterSecretStore: k8s-yana-stocks<br/>(Kubernetes provider — cross-ns secret sync)"]
...
The `k8s-yana-stocks` ClusterSecretStore (Kubernetes provider) allows secrets to be synced cross-namespace within the cluster — used to pull the CNPG-generated `auth-service-pg-app` secret from the `yana-stocks` namespace into `monitoring` for the Grafana PostgreSQL datasource. Auth via a minimal-permission ServiceAccount (`eso-pg-reader`) in the `monitoring` namespace.
...
Sentry SDK integration exists in 15 app services across 6 repos (org `yanatech-tech-limited`): yanatech, akan, dove-house-tt, shared-services' email-api/email-service, ml's k8s-docs/ops-agent, and yana-stocks' frontend/auth-service/profile-service/portfolio-service/portfolio-api/price-processor/price-ingestor/sentiment-analyzer/ml-predictor. (A prior version of this doc listed only 11 — it missed yana-stocks' auth-service, price-ingestor, sentiment-analyzer, ml-predictor, and ml/ops-agent, all of which were already wired up in code.)

**As of 2026-08-02, Sentry is active for yana-stocks only** — every other repo's free-tier quota consumption is disabled. This is a code-level, git-tracked toggle, not a change to any DSN/secret: each disabled service reads a new `SENTRY_ENABLED` env var (`NEXT_PUBLIC_SENTRY_ENABLED` for the 3 Next.js apps, since it's inlined at build time) that gates `Sentry.init`/`withSentryConfig`/the NestJS `SentryModule`, defaulting to `"false"`. DSNs and all existing Infisical/CI secret wiring are left untouched. Re-enabling any repo is a one-line flip of that flag back to `"true"` in the owning repo's CI workflow (`NEXT_PUBLIC_SENTRY_ENABLED`/`SENTRY_ENABLED` build-arg) and/or k8s `deployment.yaml` (`SENTRY_ENABLED` env var) — no Infisical changes needed. shared-services and ml/k8s-docs also had their CI "Upload source maps to Sentry" steps gated off (`if: false`) for the same reason — those run independently of the app-level flag. See each disabled repo's own CI workflow / `deployment.yaml` for the exact flag location.

Each project has a per-project issue alert rule (fires on every new issue, `environment=production` only) whose action posts to `sentry-gotify-bridge` — a small Go relay (source: `apps/gotify/sentry-bridge/`, image built by `.github/workflows/build-sentry-gotify-bridge.yaml` on push to that path) that verifies the Sentry webhook HMAC signature and forwards a formatted message to Gotify's `/message` API. Reachable at `https://gotify.yanatech.co.uk/webhook/sentry` (separate `Ingress` in `apps/gotify/manifests/sentry-bridge.yaml`, same host as the main Gotify UI, different path — same pattern as the existing `gotify-api` Ingress). Uses its own Gotify application token (`gotify-secret` key `sentry-token`) so Sentry pushes are visually distinguishable from Alertmanager pushes in the Gotify client. Disabling a project's event flow doesn't remove its alert rule automatically — that's a separate Sentry.io UI/API action, not required for quota conservation since a project with no incoming events simply won't fire it.
...
When adding new cross-namespace connections from Grafana (or any component in monitoring), create a corresponding `CiliumNetworkPolicy` in `infrastructure/cilium/` and add it to `kustomization.yaml`.
